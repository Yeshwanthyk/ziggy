...
test("S1 filesystem World assigns replay sequence and fails loud on a torn tail", async () => {
  ...
  try {
    ...
    await runEffect(
      world.appendSession("fixture-session", {
    ...
    expect(await runEffect(world.readSession("fixture-session", 1))).toEqual([second]);

    const path = join(profile, "sessions/fixture-session.ndjson");
    const before = await readFile(path, "utf8");
    await appendFile(path, '{"schemaVersion":1');
    const after = await readFile(path, "utf8");
    await expect(runEffect(world.readSession("fixture-session", 0))).rejects.toThrow();
