...
import { parseAutomationFile, parseAutomationTarget, validateAutomationId } from "./automation";
...
const parse = async (fields: ReadonlyArray<string>, body?: string) => {
  ...
  return Effect.runPromise(parseAutomationFile(id, "/profile/automations/daily-note.md", source(fields, body)));
};
...
describe("automation definition", () => {
  test("parses the exact contract independent of field order", async () => {
    ...
    expect({
      ...
      broadcast: automation.broadcast.map((token) => typeof token === "string" ? token : token.target),
      origin: automation.origin?.target,
      prompt: automation.prompt,
    }).toEqual({
      id: "daily-note", version: 1, cronSource: "0 9 * * *", timezone: "America/New_York",
      gate: "test -f READY", broadcast: ["origin", "all", "telegram:chat:-1001234567890"],
      origin: "slack:channel:C0123ABCDE:thread:1712345678.123456", prompt: "Write the daily note.",
    });
  });
  ...
  test("rejects invalid schedules and noncanonical timezone values", async () => {
    ...
    const messages = await Promise.all(cases.map((fields) => invalidMessage([...fields, "broadcast: none"])));
    expect(messages.every((message) => message.startsWith("invalid automation daily-note:"))).toBe(true);
  });

  test("accepts every canonical target form and rejects other spellings", async () => {
    const valid = [
      "telegram:chat:42", "telegram:chat:-1001234567890", "discord:channel:1234567890",
      "slack:channel:C0123ABCDE", "slack:channel:G0123ABCDE:thread:1712345678.123456",
    ];
    const parsed = await Promise.all(valid.map((value) => Effect.runPromise(parseAutomationTarget("x", "/x", value))));
    expect(parsed.map((value) => value.target)).toEqual(valid);
    const invalid = [
      "telegram:chat:0", "telegram:chat:+42", "telegram:chat:042", "telegram:chat:9007199254740992",
      "discord:channel:0", "discord:channel:01", "slack:channel:c0123ABCDE",
      "slack:channel:C0123ABCDE:thread:1.2", "telegram:42",
    ];
    const results = await Promise.all(invalid.map((value) => Effect.runPromise(parseAutomationTarget("x", "/x", value).pipe(
      Effect.as("valid"), Effect.catchTag("AutomationInvalid", () => Effect.succeed("invalid")),
    ))));
    expect(results).toEqual(invalid.map(() => "invalid"));
  });

  test("enforces policy grammar and a persisted origin", async () => {
    const invalid = ["", "origin", "none,all", "all,", ",all", "all, telegram:chat:1", "all,,origin"];
    const messages = await Promise.all(invalid.map((broadcast) => invalidMessage([
      "cron: 0 9 * * *", "timezone: UTC", `broadcast: ${broadcast}`,
    ])));
    expect(messages.every((message) => message !== "success")).toBe(true);
    const duplicate = await parse(["cron: 0 9 * * *", "timezone: UTC", "broadcast: telegram:chat:1,telegram:chat:1"]);
    expect(duplicate.broadcast.map((token) => typeof token === "string" ? token : token.target)).toEqual(["telegram:chat:1", "telegram:chat:1"]);
  });
  ...
  test("rejects strict frontmatter violations and prompt shadowing", async () => {
    ...
    expect(messages.every((message) => message !== "success")).toBe(true);
    expect(await invalidMessage(["cron: 0 9 * * *", "timezone: UTC", "broadcast: none"], " ")).not.toBe("success");
  });

  test("returns the direct telegram-chat replacement error", async () => {
    expect(await invalidMessage(["cron: 0 9 * * *", "timezone: UTC", "broadcast: none", "telegram-chat: 42"])).toBe(
