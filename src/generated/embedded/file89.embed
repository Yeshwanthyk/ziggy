...
This skill ships two helpers. Resolve both paths relative to the directory containing this `SKILL.md`, then run them with Bash:

- `scripts/find-sessions.sh` — list candidate tmux sessions/panes
- `scripts/wait-for-text.sh` — poll a pane until text appears (or timeout)
