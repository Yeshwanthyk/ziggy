...
export function parseLookResponse(raw) {
    const record = isRecord(raw) ? raw : {};
    ...
    if (!look.lookId)
        throw new Error("Helper returned a look without lookId.");
    return look;
}
function buildOutline(lookId, root) {
    const nodes = [];
    const refToWireRef = new Map();
    const wireRefToRef = new Map();
    const queue = [root];
    let index = 0;
...
export function nodeByRef(outline, ref) {
    return outline.nodes.find((node) => node.ref === ref || node.wireRef === ref);
}
export function outlineNodeLabel(node) {
...
export function outlineNodePath(node) {
    const parts = [];
    let current = node;
    while (current) {
...
export function foldToBudget(outline, budget = {}, unfoldPaths = []) {
    ...
    for (const ref of unfoldPaths) {
        ...
        if (!node)
            continue;
        for (const pathRef of pathRefs(node))
            unfolded.add(pathRef);
        for (const child of node.children)
            unfolded.add(child.ref);
    }
    const lines = [];
    const renderedRefs = [];
    let truncated = false;
...
function actionMatches(node, action) {
    ...
    if (query === "press" && node.canPress)
        return true;
    if (query === "focus" && node.canFocus)
        return true;
    if ((query === "setvalue" || query === "set_text" || query === "settext") && node.canSetValue)
        return true;
    if (query === "scroll" && node.canScroll)
        return true;
    if (query === "increment" && node.canIncrement)
        return true;
    if (query === "decrement" && node.canDecrement)
        return true;
...
export function rankedTextMatch(values, text) {
    ...
    if (candidates.some((value) => value === query))
        return { reason: "exact", score: 1 };
    if (candidates.some((value) => value.startsWith(query) || value.split(/\W+/).some((token) => token.startsWith(query))))
        return { reason: "prefix", score: 0.95 };
    if (candidates.some((value) => value.includes(query)))
        return { reason: "substring", score: 0.9 };
    let score = 0;
...
export function searchOutlineRanked(outline, text, role, capability, limit = 12) {
    ...
    for (const [order, node] of outline.nodes.entries()) {
        if (roleQuery && normalizedSearchRole(node.role) !== roleQuery)
            continue;
        if (actionQuery && !actionMatches(node, actionQuery))
            continue;
        const label = outlineNodeLabel(node);
        ...
        if (query && !match)
            continue;
        const result = { ref: node.ref, role: node.role, label, actions: node.actions, path: outlineNodePath(node), matchReason: match?.reason ?? "filter", score: match?.score ?? 1, node, order };
    ...
    if (useFuzzy)
        sorted.push(...fuzzy.sort((a, b) => b.score - a.score || a.order - b.order));
    return { matches: sorted.slice(0, limit).map(({ order: _order, ...match }) => match), totalMatches: strong.length + (useFuzzy ? fuzzy.length : 0) };
}
export function searchOutline(outline, text, role, action, limit = 50) {
    ...
    for (const node of outline.nodes) {
        ...
        if (query && !haystack.includes(query))
            continue;
        if (roleQuery && node.role !== roleQuery)
            continue;
        if (actionQuery && !actionMatches(node, actionQuery))
            continue;
...
export function serializeOutlineSearchMatch(match) {
    const { node: _node, ...serialized } = match;
    return serialized;
}
export function countOutlineNodes(root) {
    let count = 1;
...
export function serializeOutlineNodeShallow(node) {
    const { parent: _parent, children: _children, ...rest } = node;
    return { ...rest, children: [] };
}
export function restoreOutline(serialized) {
    const nodes = [];
    const refToWireRef = new Map();
    const wireRefToRef = new Map();
...
export function graftScopedOutline(outline, targetRef, scoped) {
    const target = nodeByRef(outline, targetRef);
    if (!target)
        throw new Error(`Cannot graft scoped outline: target ${targetRef} is not in the current outline.`);
    const targetRect = target.rect ? { ...target.rect } : undefined;
    const reusableByWireRef = new Map();
    const collect = (node) => {
        if (node.wireRef)
            reusableByWireRef.set(node.wireRef, node);
        for (const child of node.children)
            collect(child);
    };
    collect(target);
