...
```sh
ziggy memory list [<profile>] [--json]
ziggy memory show <profile> shared [--json]
ziggy memory show <profile> user:<id> [--json]
ziggy memory show <profile> group:<id> [--json]
```

Without a target, `memory list` inspects the current directory as the Profile target. JSON output
contains scope, path, state, entry count, code-point count, and cap, but not raw memory content for
`list`; `show --json` includes the bounded entries of the selected document.
