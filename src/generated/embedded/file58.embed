...
Use HyperFrames for motion graphics, talking-head videos with captions, product tours, social
overlays, shader transitions, and anything driven by real video/audio media. Use a dedicated
mathematical animation tool for equation-heavy or geometric explainers.
...
Resolve `scripts/setup.sh` relative to this loaded `SKILL.md`; do not search home directories for
another copy of the skill.
...
4. **Visual check:** extract a mid-composition frame: `ffmpeg -i final.mp4 -ss 00:00:05 -vframes 1 preview.png`.
5. **Audio present if expected:** `ffprobe -v error -show_streams -select_streams a -of default=nw=1:nk=1 final.mp4 | head -1`.
...
- [troubleshooting.md](references/troubleshooting.md) — environment variables and common render errors
