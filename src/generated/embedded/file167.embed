...
qmd exposes an MCP server for clients that support MCP. Configure it in the client using that
client's documented MCP settings.

### Option A: Stdio Mode (Simple)

Configure an MCP stdio server with this command and argument:
...
Then configure the MCP client to connect via HTTP:
...
Tool names may be namespaced by the MCP client:

| MCP Tool | Maps To | Description |
|----------|---------|-------------|
| `search` | `qmd search` | BM25 keyword search |
| `vsearch` | `qmd vsearch` | Semantic vector search |
| `deep_search` | `qmd query` | Hybrid search + reranking |
| `get` | `qmd get` | Retrieve document by ID or path |
| `status` | `qmd status` | Index health and stats |
...
When MCP is not configured, use qmd directly:

```bash
qmd query "what was decided about the API redesign" --json
```

For setup and management tasks, use the CLI:

```bash
qmd collection add ~/Documents/notes --name notes
qmd context add qmd://notes "Personal research notes and ideas"
qmd embed
qmd status
```
...
5. **Prefer MCP integration when the current client supports it** — once configured, searches can
