...
import { resolveMacosHelperAppPath } from "../dist/src/platform/macos/helper-path.mjs";
...
async function installPrebuiltHelperApp(sourceAppPath) {
	...
	// Ziggy's embedded-file materializer writes regular files without preserving
	// executable bits. Restore the signed helper's launch bit before verification
	// and ditto copy; chmod does not alter the code-signed bytes.
	await fs.chmod(sourceExecutablePath, 0o755);
