...
Browser access requires a stable port configured before pairing. Tailscale is optional for remote
access, and its background mapping does not need to be recreated for every browser or resident
restart. See [Use Ziggy from a browser](web-access.md) for local and remote setup, pairing,
revocation, shared-Profile scope, and troubleshooting.
