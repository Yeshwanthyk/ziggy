import { serializeOutlineNode } from "./outline.mjs";
...
export function stabilizeRefs(base, next) {
    if (!base)
        return next;
    ...
    for (const node of next.nodes) {
        if (assigned.has(node))
            continue;
        while (reserved.has(`@e${nextIndex}`))
            nextIndex += 1;
...
export function changesBetween(base, next) {
    ...
    for (const node of next.nodes) {
        ...
        if (!previous)
            changes.push({ type: "added", ref: node.ref, parent: node.parent?.ref, node: { ...serializeOutlineNode(node), children: [] } });
        else {
            const fields = changedFields(previous, node);
            if (Object.keys(fields).length > 0)
                changes.push({ type: "updated", ref: node.ref, path: refPath(node), fields });
        }
    }
    for (const node of base.nodes)
        if (!after.has(node.ref))
            changes.push({ type: "removed", ref: node.ref, parent: node.parent?.ref });
...
export function renderChanges(changes) {
    return changes.map((change) => {
        if (change.type === "added")
            return `+ ${change.ref}${change.parent ? ` under ${change.parent}` : ""} ${JSON.stringify(label(change.node))}`;
        if (change.type === "removed")
            return `- ${change.ref}${change.parent ? ` from ${change.parent}` : ""}`;
