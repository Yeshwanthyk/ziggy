...
Use that known path directly; do not search home directories for another copy of the skill.

## Create a site

```bash
SKILL_DIR="<absolute directory containing the loaded SKILL.md>"
bash "$SKILL_DIR/scripts/publish.sh" {file-or-dir}
```
...
```bash
SKILL_DIR="<absolute directory containing the loaded SKILL.md>"
bash "$SKILL_DIR/scripts/drive.sh" default
bash "$SKILL_DIR/scripts/drive.sh" ls "My Drive"
bash "$SKILL_DIR/scripts/drive.sh" put "My Drive" notes/today.md --from ./notes/today.md
bash "$SKILL_DIR/scripts/drive.sh" cat "My Drive" notes/today.md
bash "$SKILL_DIR/scripts/drive.sh" share "My Drive" --perms write --prefix notes/ --ttl 7d
```
...
| `--client {name}`      | Client name for attribution                   |
