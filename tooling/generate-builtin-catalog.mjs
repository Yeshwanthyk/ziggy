...
const walkFiles = (root) => {
  ...
  return readdirSync(root, { withFileTypes: true }).flatMap((entry) => {
    if (skipNames.has(entry.name) || entry.name.endsWith(".test.ts")) return [];
...
for (const id of approvedIds) {
  ...
  packages.push({
    ...
    sourcePath: posix(relative(repositoryRoot, packagePath)),
...
for (const pkg of packages) {
  ...
  for (const skill of pkg.skills) {
    for (const file of skill.files) {
      const alias = importAlias("file", `${pkg.id}${fileIndex}`, 0);
      fileIndex += 1;
      skillAlias.set(file, alias);
      fileImports.push(`import ${alias} from "../../${file}" with { type: "file" };`);
    }
  }
  for (const automation of pkg.automations) {
    if (!skillAlias.has(automation.logicalPath)) {
      const alias = importAlias("file", `${pkg.id}Automation`, 0);
      skillAlias.set(automation.logicalPath, alias);
      fileImports.push(
        `import ${alias} from "../../${automation.logicalPath}" with { type: "file" };`,
      );
    }
  }
}
for (const skill of extraSkills) {
  for (const file of skill.files) {
    const alias = importAlias("file", skill.id, fileIndex);
    fileIndex += 1;
    skillAlias.set(file, alias);
    fileImports.push(`import ${alias} from "../../${file}" with { type: "file" };`);
  }
