...
/* oxlint-disable ziggy-effect/no-json-parse -- boundary: adapter validates the complete persisted journal immediately */
...
/* oxlint-disable ziggy-effect/no-instanceof-tagged-error -- boundary: adapter identifies its private native conflict sentinel */
...
class NodeMemoryBatchConflictError extends Error {}
...
export interface FilesystemWorldOptions {
  ...
  readonly onMemoryCommitPoint?: (point: MemoryCommitCutPoint) => void;
  readonly onMemoryRecoveryPoint?: (point: MemoryRecoveryPoint) => void;
  readonly onSessionAppendPoint?: (point: SessionAppendPoint) => void;
}
...
export function createNodeFilesystemWorld(options: FilesystemWorldOptions): FilesystemWorld {
  ...
  const onMemoryCommitPoint = options.onMemoryCommitPoint ?? (() => undefined);
  const onMemoryRecoveryPoint = options.onMemoryRecoveryPoint ?? (() => undefined);
  const onSessionAppendPoint = options.onSessionAppendPoint ?? (() => undefined);

  return {
    readSessionSnapshot(sessionId) {
      return nodeOperation("Failed to read Session snapshot", async () => {
        validateSessionId(sessionId);
        return runNodeOperation(async () => {
    ...
    startSession(sessionId, snapshot) {
      return nodeOperation("Failed to start Session", async () => {
        validateSessionId(sessionId);
        return runNodeOperation(async () => {
    ...
    appendSession(sessionId, event) {
      return nodeOperation("Failed to append Session event", async () => {
        validateSessionId(sessionId);
        return runNodeOperation(async () => {
    ...
    readSession(sessionId, afterSeq) {
      return nodeOperation("Failed to read Session", async () => {
        validateSessionId(sessionId);
    ...
    listSessions() {
      return nodeOperation("Failed to list Sessions", () =>
        runNodeOperation(async () => {
          await ensureSafeDirectory(profilePath, false);
          ...
          const sessions = await Promise.all(
            entries.filter(isSessionEntry).map(async (entry) => {
              ...
              validateSessionId(sessionId);
              const validated = await runNodeOperation(() =>
    ...
    readMemory(document) {
      return nodeOperation("Failed to read Memory document", async () => {
    ...
    readMemoryBatch(documents) {
      return nodeOperation("Failed to read Memory batch", async () => {
    ...
    replaceMemoryBatch(replacements, expected) {
      return nodeOperationPreservingConflict("Failed to replace Memory batch", async () => {
        ...
        return runNodeOperation(async () => {
          ...
          for (const expectation of expectations) {
            if (
              ...
              throw new NodeMemoryBatchConflictError();
            }
          }
...
function nodeOperationPreservingConflict<Value>(
  _operationName: string,
  operation: () => Promise<Value>,
): Promise<Value> {
  return operation();
...
export function isNodeMemoryBatchConflictError(
  cause: unknown,
): cause is NodeMemoryBatchConflictError {
  return cause instanceof NodeMemoryBatchConflictError;
}

export function nodeFilesystemErrorMessage(cause: unknown, fallback: string): string {
  return cause instanceof Error && cause.message.length > 0 ? cause.message : fallback;
}

function runNodeOperation<Value>(operation: () => Promise<Value>): Promise<Value> {
  return operation();
}
...
async function recoverMemory(
  ...
  onMemoryRecoveryPoint: (point: MemoryRecoveryPoint) => void,
): Promise<void> {
  if (!(await ensureSafeDirectoryIfPresent(memoryPath))) {
    return;
  }
