...
5. **Extension** — `catalog.json` is the sole approved public catalogue. Approved packages are compiled into the Ziggy executable; `<profile>/extensions/<id>/` holds Profile-owned packages. `<profile>/extensions.json` is the sole active-selection record, and a Profile-owned package wins over an approved bundled package with the same ID. `pi-packages`, `extension-authoring`, and `ziggy-operations` remain mandatory. CLI and TUI share one select/deactivate lifecycle, and all faces share one runtime resolver. _Proof:_ missing, invalid, and unapproved selections fail closed; Profile-owned packages and skills win collisions; selected Profile-owned tools load from direct paths; bundled tools load from compile-in factories; catalogue list/show use generated metadata; and selection writes are atomic.
