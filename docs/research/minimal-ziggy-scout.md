...
**Ziggy owns:** Profile discovery/init policy; which Pi resources are admitted; `SOUL.md`, `MEMORY.md`, `USER.md`, automation files, later channel routing; Effect services and typed product errors; process ownership once a gateway exists; command naming and user-facing composition. Ziggy extensions are skills. A fixed internal Pi TypeScript extension only shapes Pi's TUI into the Ziggy surface.
...
4. `DefaultResourceLoader` with `systemPrompt: join(profilePath, "SOUL.md")`; set `noExtensions`, `noSkills`, `noPromptTemplates`, `noThemes`, `noContextFiles`, then admit Profile-local skills before every live sibling Merlin skill root. No accidental global/project discovery or Profile-authored TypeScript extensions.
...
4. **Memory** — retained facts separate from transcripts, all plain markdown in the Profile. `MEMORY.md` is assistant-wide; `memory/users/<id>.md` is per-person and loaded only in 1:1 contexts; `memory/groups/<id>.md` is shared per group and is the only extra memory loaded in group contexts — individual user memories never leak into groups. Capped, reject-on-overflow, never silent truncation. *Proof:* a Ziggy-owned Pi tool atomically replaces a bounded memory doc mid-chat; the next session sees the fact via prompt context; transcript untouched. No indexes, embeddings, or background dreaming.
5. **Extension** — Ziggy extensions are Agent Skills. Pi receives Profile-local skills first, then every `../merlin/extensions/*/skills` root, then `../merlin/skills`; full `SKILL.md` bodies load on demand. One fixed Ziggy-owned Pi TypeScript extension shapes the TUI header, footer, and title. *Proof:* Profile and Merlin skills are visible in the TUI with Profile-local collision precedence; nothing global leaks in; no user-authored Pi extension is loaded.
