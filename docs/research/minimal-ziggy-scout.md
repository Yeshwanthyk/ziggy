...
Every production Profile agent execution is a saved Pi session. A direct agent or tagged automation run is one saved root; an `agent_run` or `agent_discuss` participant is a saved child whose Pi header points to the parent session file. The parent Pi tool result keeps only bounded output, nested usage, and a child session reference. The isolated child JSONL keeps the complete transcript. Ziggy does not copy transcripts or maintain a second session registry.
