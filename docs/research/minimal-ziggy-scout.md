...
**Ziggy owns:** Profile discovery/init policy; which Pi resources are admitted; `SOUL.md`, `MEMORY.md`, `USER.md`, automation files, later channel routing; Effect services and typed product errors; process ownership once a gateway exists; command naming and user-facing composition; and the repository-owned Pi package catalog under `extensions/`. A package may contain progressively loaded Agent Skills, executable Pi extensions, or both. Repository packages are inactive until the user adds their resources to a Profile. One hidden internal Pi extension shapes the TUI.
...
4. `DefaultResourceLoader` with `systemPrompt: join(profilePath, "SOUL.md")`; set `noSkills`, `noPromptTemplates`, `noThemes`, and `noContextFiles`. Pi admits only extensions the user added to the Profile through its Profile-local package settings or `extensions/` directory. Ziggy explicitly admits only `<profile>/skills`; repository package and top-level skills remain catalog sources until the user installs them into that directory.
...
5. **Extension** — each repository `extensions/<id>/` folder is an available Pi package containing skills, executable extension code, or both. A new Profile loads no visible extensions or skills. Pi loads only extensions the user added to that Profile; Ziggy loads only skills installed under `<profile>/skills`, with full `SKILL.md` bodies loaded on demand. *Proof:* an empty Profile has no visible resources, one Profile-added extension and one installed skill load, repository catalog packages stay inactive, and the hidden TUI extension still shapes only terminal chrome.
