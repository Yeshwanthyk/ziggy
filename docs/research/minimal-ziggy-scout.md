...
**Pi owns:** loop execution, model/provider implementations, auth mechanics, session JSONL (append-only tree, compaction, branching), tool execution, resource loading, extension lifecycle, skill parsing, event stream, interactive TUI, print mode, RPC mode.

**Ziggy owns:** Profile discovery/init policy; which Pi resources are admitted; `SOUL.md`, `MEMORY.md`, `USER.md`, automation files, later channel routing; Effect services and typed product errors; process ownership once a gateway exists; command naming and user-facing composition.
...
4. `DefaultResourceLoader` with `systemPrompt: join(profilePath, "SOUL.md")`; set `noExtensions`, `noSkills`, `noPromptTemplates`, `noThemes`, `noContextFiles`, then admit explicit Profile paths as each capability lands. No accidental global/project discovery.
...
5. **Extension** — Pi-native Profile `skills/` and `extensions/`; no new manifest, registry, or installer. *Proof:* a Profile-scoped `SKILL.md` is visible and invocable in the TUI; nothing global leaks in. TypeScript extensions (loop hooks) come after the markdown path works.
