...
| Part | Mechanism                                                                                                                                                                                                                                          |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S1   | A fixed HTTPS URL points to `catalog.json` on `Yeshwanthyk/ziggy`. `extensions list`, `extensions show`, `extensions add`, and the TUI fetch and Schema-decode it. Runtime startup does not fetch it. |
...
| S5   | Bundled packages, including required `pi-packages`, `extension-authoring`, and `ziggy-operations`, are compiled file trees. `extensions add` and runtime start copy them onto the Profile. Runtime never loads `$bunfs` skill paths.              |
