...
- [x] Two different sessions' turns can run concurrently without one's tool execution blocking the other's event delivery.
...
- [x] `ziggy service install` produces a working launchd plist (macOS) or systemd unit (Linux) that starts the daemon; `ziggy doctor` correctly reports daemon-up/down and a stale lock left by a killed daemon.
