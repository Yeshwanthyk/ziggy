...
| Extension | Ziggy extensions are skills: Profile-local skills override every live Merlin skill root; Pi reads bodies on demand. One fixed internal Pi extension shapes the TUI. | Rich extension tree and the source catalog. | Sync, provenance, quarantine, and lifecycle machinery. | Multi-root discovery and hardened installation. | Manifest-driven extensions and supervised commands. | Complete for current scope. No user-authored Pi extension host. Defer remove/update/provenance; bundle only when sibling Merlin is no longer acceptable. |
