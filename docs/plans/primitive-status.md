...
| Automation | Profile-owned Markdown definitions and bounded Markdown receipts; unified Run now/scheduled admission; cron/at/every; foreground scheduler; launchd and systemd-user services; scheduler/next/latest/run-history TUI projections; local-first Telegram, Discord, and Slack outcomes. | V1-V6 shipped. Keep retries, replay, Windows services, webhooks, and unbounded history out. |
