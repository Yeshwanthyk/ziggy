...
| Extension | All 47 repository-owned `extensions/<id>/` folders are Pi packages containing skills, executable extension code, or both. Only Profile-installed skills load; `/skills` marks installed entries, multi-selects from 57 catalog skills, and reloads the TUI once after installation. All 19 package tools run in every face. One hidden internal Pi extension shapes the TUI. | Complete. Pi remains the only extension host; do not add a parallel registry or load Profile-authored executable code. |
