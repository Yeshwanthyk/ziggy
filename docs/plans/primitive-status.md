...
| Extension     | Repository-root `catalog.json` is the sole approved catalogue. Bundled and Profile-owned `<profile>/extensions/<id>/` folders store Pi package payloads containing skills, executable extension code, or both. Selected Profile-owned packages take precedence by ID, and Profile skills load first. CLI and TUI share one install/select/deactivate lifecycle. Package tools run in every face. One hidden internal Pi extension shapes the TUI.                                                                                                                     | Complete for approved bundled, checksum-pinned GitHub, and Profile-local package admission. Pi remains the only extension host.                                         |
