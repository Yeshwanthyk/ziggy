...
function refinementFor(tool) {
    if (tool === "search_ui")
        return "use a more selective text, role, or capability predicate";
    if (tool === "find_roots")
        return "use a more selective text, app, bundleId, pid, or kind filter";
    if (tool === "evaluate_browser")
        return "return selected fields, an aggregate, or a smaller slice";
    if (tool === "observe_ui" || tool === "expand_ui")
        return "use search_ui or expand a more specific ref";
    return "request a smaller or more focused result";
}
export function applyOutputEnvelope(tool, result) {
    ...
    if (bytes <= MODEL_TEXT_MAX_BYTES && lines <= MODEL_TEXT_MAX_LINES)
        return result;
...
export function boundToolError(tool, error) {
...
export function readStoredOutput(ref, offsetValue) {
