...
import { HELPER_APP_PATH, macosHelper } from "./helper.mjs";
import { assertPlatformArchitecture } from "../architecture.mjs";
const GRANT_INSTRUCTIONS = "Grant Accessibility and Screen Recording to pi-computer-use.app in System Settings → Privacy & Security. " +
    "Screen Recording lets the agent see the window; Accessibility lets it interact with the window.";
const SIGNING_MIGRATION_WARNING = "If these permissions were enabled before this install/update, macOS invalidated the old grants because " +
...
function permissionStatusSummary(status) {
    ...
    if (status.screenRecordingPreflight && !status.screenRecording) {
        lines.push("(Screen Recording reads granted in the TCC database but a live capture probe failed — " +
...
export async function ensureMacosReady(ctx, state, signal) {
    ...
    const canUseCachedPermissions = cachedStatus?.accessibility &&
    ...
    if (!permissionStatus.accessibility || !permissionStatus.screenRecording) {
        ...
        const attributionHint = permissionStatus.source?.attribution === "caller"
            ...
            : undefined;
        permissionStatus = await ensurePermissions(ctx, {
