...
export async function runProcess(command, args, timeoutMs, signal, env) {
    throwIfAborted(signal);
    await new Promise((resolve, reject) => {
...
export class MacosHelperClient {
    ...
    async launchDaemon(signal) {
        if (usingExternalHelperSocket)
            throw new HelperTransportError(`External helper socket is unavailable at ${HELPER_SOCKET_PATH}.`);
            ...
            socket.on("data", (chunk) => {
                ...
                try {
                    ...
                    if (parsed.ok === true)
                        ...
                        reject(new HelperCommandError(parsed?.error?.message ?? `Daemon command '${cmd}' failed.`, parsed?.error?.code));
                }
                catch (error) {
                    reject(error);
                }
    ...
    async ensureProtocol(signal) {
        ...
        if (diagnostics.protocolVersion !== HELPER_PROTOCOL_VERSION || !relaunchedExecutableMatches) {
            this.daemonAvailable = false;
            throw new Error(`pi-computer-use helper mismatch after relaunch: expected protocol ${HELPER_PROTOCOL_VERSION} and executable ${HELPER_APP_EXECUTABLE_PATH}; got protocol ${diagnostics.protocolVersion} and executable ${diagnostics.executablePath ?? "unknown"}. Reinstall or rebuild the helper app at ${HELPER_APP_PATH}.`);
        }
        return diagnostics;
    }
}
