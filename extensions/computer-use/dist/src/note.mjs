import { nodeByRef, outlineNodeLabel } from "./outline.mjs";
...
function topLevelAncestor(node, outline) {
    let current = node;
    while (current.parent && current.parent !== outline.root)
...
export function noteFromLook(prev, outline, window) {
...
export function noteAfterAct(prev, targetRef, outline, refreshOutcome) {
...
export function noteRegionKeyForRef(outline, ref) {
...
export function renderNote(note) {
    ...
    if (note.regions.length > visible.length)
        lines.push(`  … ${note.regions.length - visible.length} more note regions; use search_ui or expand_ui`);
