...
The preferred boundary is schema-first. Define the schema once, export `type X = typeof XSchema.Type`, and make domain code consume the inferred type. This prevents drift between parsing and static types.
...
- Use the schema's `Type` member for non-exported schemas too:

```ts
type IntrospectionResult = typeof IntrospectionResultModel.Type;
```
