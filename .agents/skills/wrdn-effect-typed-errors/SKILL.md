...
The preferred boundary is typed `Schema.TaggedErrorClass` / `Data.TaggedError` values in the Effect error channel. Construct the tagged error directly at the failure site unless a helper performs real classification or normalization.

## Trace before changing

1. **Identify the boundary.** Is this Effect domain code, a third-party callback, or plain test/tooling code?
2. **Find the existing domain errors.** Check nearby `errors.ts`, `Schema.TaggedErrorClass`, and `Data.TaggedError` declarations before adding a new class.
...
```ts
// oxlint-disable-next-line ziggy/no-try-catch-or-throw -- boundary: JSON.parse feeds stable IPC failure envelope
try {
...
If cleanup cannot propagate its failure, handle and report it explicitly:

```ts
Effect.tryPromise({
  try: () => client.close(),
  catch: (cause) => new ClientCloseError({ cause }),
}).pipe(Effect.catch((error) => Effect.logError("Client cleanup failed", { error })));
```

### Effect failure-erasure escape hatches
...
program.pipe(Effect.ignoreCause);
```
...
`Effect.die`, `Effect.dieMessage`, `Effect.orDie`, and `Effect.orDieWith` turn typed failures into defects. `Effect.ignore` and `Effect.ignoreCause` silently erase them. Keep failures typed whenever they can propagate; otherwise catch them explicitly and report, aggregate, or recover. Do not suppress this rule to avoid threading an error type through normal Effect code.
...
```ts
const decodeInputJson = Schema.decodeUnknownEffect(Schema.fromJsonString(InputSchema));

decodeInputJson(text).pipe(
